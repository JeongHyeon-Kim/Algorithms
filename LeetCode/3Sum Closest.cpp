...
// discussion reference: https://leetcode.com/problems/3sum-closest/discuss/7987/12-lines-concise-and-easy-understand-c++-solultion
class Solution {
public:
    int threeSumClosest(vector<int>& nums, int target) {
        int result = nums[0] + nums[1] + nums[2]; // because of condition 'count is more than 3'
        ...
        for (int i = 0; i < nums_size; i++) {
            ...
            int k = nums_size - 1;
            while (j < k) {
                int middle_result = nums[i] + nums[j] + nums[k];
                if (abs(middle_result - target) < abs(result - target)) // get closest value
                    result = middle_result;
                if (middle_result < target)
                    j++; // front to back
                else
                    k--; // back to front
            }
