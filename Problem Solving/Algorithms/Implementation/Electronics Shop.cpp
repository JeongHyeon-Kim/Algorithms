// problem source: https://www.hackerrank.com/challenges/electronics-shop/problem
#include <bits/stdc++.h>
...
int getMoneySpent(vector<int> keyboards, vector<int> drives, int b) {
     ...
     */
    int keyboards_count = keyboards.size();
    int drives_count = drives.size();
    int result = -1;
    for (int i = 0; i < keyboards_count; i++) {
        for (int j = 0; j < drives_count; j++) {
            if (keyboards[i] + drives[j] <= b && result < keyboards[i] + drives[j])
