...
int birthdayCakeCandles(vector<int> candles) {
    int candles_size = candles.size();
    unordered_map<int, int> mapping; // number, count
    int max_number = 0, count_for_max = 0;
    for (int i = 0; i < candles_size; i++) {
        mapping[candles[i]]++;
    }
    for (pair<int, int> atom : mapping) {
        // cout << atom.first << " " << atom.second << endl;
        if (max_number < atom.first) {
            max_number = atom.first;
            count_for_max = atom.second;
        }
    }
    return count_for_max;
}
